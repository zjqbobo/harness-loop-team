...
```
┌──────────────────────┐     HTTP/gRPC      ┌──────────────────────┐
│   企业 Java 后端       │ ◄────────────────► │   Agent 服务 (Python) │
│                       │                    │                      │
│  Spring Boot / K8s    │   POST /agent/run  │   LangGraph 编排引擎   │
│  业务逻辑 + 数据库     │   GET  /agent/{id} │   模型网关 + RAG      │
│  权限 + 工作流         │                    │   技能执行 + 记忆     │
└──────────────────────┘                    └──────────────────────┘

完整链路:

用户 → Java 后端 (鉴权+业务上下文注入)
  → Higress Gateway (限流+Prompt Injection检测+灰度)
  → Agent Router (意图分类→决策路由)
  → Agent 服务 Python (LangGraph编排)
  → Java 后端 (格式化→审计→返回前端)
```
