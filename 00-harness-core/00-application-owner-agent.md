...
> ⚠️ **🔴 版本状态（必读）**：本文档为 **V3 架构愿景**（10 阶段全流程），**不是当前实装版本**。
>
> **当前实装**：`docs/pipeline-state-schema.md` 中定义的 5 阶段 schema 是**唯一权威的流水线状态定义**：
> ```
> document-complete → implementation-complete → testing-complete → code-review-complete → closed
> ```
> 所有 skill 文件（harness-implementation / harness-testing / harness-code-review 等）均以此 5 阶段 schema 为准。
>
> 本文档中的阶段 6-10（CI/部署/发布/文档更新/知识库更新）标注了 `[待实现]`，对应的 Gateway 4-5、人工确认点 #4-#5、回退路径 05-07 均为**设计稿，尚未实装**。
>
> **编码/测试/审查时**：以 `pipeline-state-schema.md` 的 5 阶段为准，不要引用本文档的 10 阶段编号。
...
1. **编排** 流水线的完整执行（当前实装 5 阶段，V3 愿景 10 阶段）
