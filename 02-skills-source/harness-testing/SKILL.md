...
<HARD-GATE>
测试执行前必须：
1. 检查流水线状态：按路径解析规则查找 `04-changes/` 下的 `pipeline-state.json`，确认当前处于 `02-implementation-complete` 阶段。如无状态文件，询问用户"未检测到编码完成记录，是否直接进行测试？"
2. 确认被测试代码的范围和依赖
...
### 门禁标准
→ 按路径解析规则查找 `00-harness-core/00-application-owner-agent.md` Gateway 3:
```json
{
...
> **是否生成测试报告？**
> - 是 → 按路径解析规则查找 `09-templates/测试报告文档模板.md` → Read 模板 → 生成测试报告(MD) → 用户审阅 → 可选转DOCX
> - 否 → 跳过

测试报告内容：执行概况（通过/失败/跳过/阻塞 + 覆盖率）、按需求维度组织的执行明细、缺陷清单、风险与遗留问题、测试结论。

### <HARD-GATE> 测试完成后更新流水线状态

测试执行完毕后（无论是否生成报告），**必须**更新变更目录下的 `pipeline-state.json`：

```json
{
  "stage": "04-testing-complete",
  "timestamp": "<ISO 8601>",
  "test_result": {
    "total": <N>,
    "passed": <N>,
